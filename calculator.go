...
func main(){
	...
	expressions := []string{"1 2 +", "1 2 3 * +", "3 2 1 - *", "3 2 1", "3 + -", " 1 2 + 3 4 - * ", "3 2 + 4 ^", "", "3 2 2 - /", "3 2 **"}
...
func calculate(expression string) (float64, error){
	...
	for _, token :=  range tokens {
		if isSupportedOperator(token){
			...
			operand2, operands = popFloat(operands)
			operand1, operands = popFloat(operands)
			//fmt.Println("Calculate ", token, operand1, operand2)
			var result float64
			switch token {
			case "+":
...
func popFloat(slice []float64) (header float64, output []float64){
	header = slice[0]
	copy(slice, slice[1:])
	return header, slice
}
